...
def retrieve_arc_wise_states_from_orbit(orbit, arc_times):
    ...
    for time in arc_times:
        ...
        for i in range(6):
            current_initial_state[i] = np.interp(
                np.array(time), state_history[:, 0], state_history[:, i + 1]
            )
...
def create_accelerations(acceleration_models, bodies, spacecraft_name):
    ...
    if "Sun" in acceleration_models:
        if acceleration_models["Sun"]["point_mass_gravity"]:
            accelerations_due_to_sun.append(
    ...
    if "Moon" in acceleration_models:
        if acceleration_models["Moon"]["point_mass_gravity"]:
            accelerations_due_to_moon.append(
                propagation_setup.acceleration.point_mass_gravity()
            )
    ...
    if "Earth" in acceleration_models:
        if acceleration_models["Earth"]["point_mass_gravity"]:
            accelerations_due_to_earth.append(
                propagation_setup.acceleration.point_mass_gravity()
            )
    ...
    if "Venus" in acceleration_models:
        if acceleration_models["Venus"]["point_mass_gravity"]:
            accelerations_due_to_venus.append(
                propagation_setup.acceleration.point_mass_gravity()
            )
    ...
    if "Mars" in acceleration_models:
        if acceleration_models["Mars"]["point_mass_gravity"]:
            accelerations_due_to_mars.append(
                propagation_setup.acceleration.point_mass_gravity()
            )
    ...
    if "Jupiter" in acceleration_models:
        if acceleration_models["Jupiter"]["point_mass_gravity"]:
            accelerations_due_to_jupiter.append(
                propagation_setup.acceleration.point_mass_gravity()
            )
...
def propagate_initial_state(
    ...
    single_arc_propagator_settings = create_propagator_settings(
        initial_state,
        initial_time,
        final_time,
        bodies,
    ...
    saved_accelerations = np.zeros(
        (np.shape(dependent_variables)[0], np.shape(dependent_variables)[1] - 8)
    )
    if save_accelerations:
...
def get_initial_states(bodies, arc_start_times, spacecraft_name):
    arc_initial_states = []
    for i in range(len(arc_start_times)):
        arc_initial_states.append(
            # ephemeris from previous propagation
            bodies.get(spacecraft_name).ephemeris.cartesian_state(arc_start_times[i])
            - bodies.get("Earth").ephemeris.cartesian_state(arc_start_times[i])
        )
    return arc_initial_states


def define_multi_arc_propagation_settings(
    arc_wise_initial_states,
    arc_start_times,
    arc_end_times,
    bodies,
    ...
    for i in range(nb_arcs):
        ...
        arc_termination_condition = (
            propagation_setup.propagator.non_sequential_termination(
        ...
        dependent_variables = []
        dependent_variables.append(
            propagation_setup.dependent_variable.total_acceleration(spacecraft_name)
        )

        accelerations = create_accelerations(
            acceleration_models, bodies, spacecraft_name
        )
    ...
    multi_arc_propagator_settings = propagation_setup.propagator.multi_arc(
        propagator_settings_list
    )
