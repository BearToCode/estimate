...
propagation_time = nb_days_to_propagate * constants.JULIAN_DAY
...
cartesian_states, keplerian_states, latitudes, longitudes, saved_accelerations = (
    propagate_initial_state(
        initial_state, initial_epoch, final_epoch, bodies, accelerations, "Delfi", True
    )
)

# Retrieve accelerations
accelerations_to_save, accelerations_ids = retrieve_accelerations_to_save(
    accelerations, "Delfi"
)
...
ax.plot(
    cartesian_states[:, 1] / 1.0e3,
    cartesian_states[:, 2] / 1.0e3,
    cartesian_states[:, 3] / 1.0e3,
    label="Delfi-C3",
    linestyle="-.",
)
...
colat_horizon = np.arccos(
    ...
    + np.sin(Lambda)
    * np.sin((90 - location_groundstation_lat) / 180 * np.pi)
    * np.cos(Phi_E % 2 * np.pi)
)
DL = mask_E * np.arccos(
    (
        np.cos(Lambda)
        - np.cos(colat_horizon)
        * np.cos((90 - location_groundstation_lat) / 180 * np.pi)
    )
...
longitude_horizon = np.where(
    longitude_horizon_abs <= 180, longitude_horizon_abs, longitude_horizon_abs - 360
)
...
ax.coastlines()
ax.scatter(
    longitudes / np.pi * 180, latitudes / np.pi * 180, label="Delfi-C3", marker=".", s=2
)
ax.scatter(
    location_groundstation_lon,
    location_groundstation_lat,
    color="red",
    marker="*",
    s=200,
)
...
Lambda_e = np.arccos(
    np.cos(latitudes[:, 1]) * np.cos(np.deg2rad(location_groundstation_lat))
    + np.sin(latitudes[:, 1])
    * np.sin(np.deg2rad(location_groundstation_lat))
    * np.cos(DL_e)
)
...
azimuth_abs = np.rad2deg(
    ...
    * np.arccos(
        (
            np.cos(np.deg2rad(location_groundstation_lat))
            - np.cos(latitudes[:, 1]) * np.cos(Lambda_e)
        )
        / (np.sin(latitudes[:, 1]) * np.sin(Lambda_e))
    )
)
...
ax.plot(
    (simulated_obs_times - start_recording_day) / 3600,
    simulated_doppler,
    color="red",
    linestyle="none",
    marker=".",
...
recording_start_times = extract_recording_start_times_yml(
    metadata_folder, metadata, old_yml=False
)
...
real_obs_single_pass = get_observations_single_pass(
    single_pass_start_time, single_pass_end_time, observations_set
)

# Retrieve simulated Doppler data for single pass
simulated_obs_single_pass = get_observations_single_pass(
    single_pass_start_time, single_pass_end_time, simulated_observations
)

# Interpolate simulated and recorded observations to identical times
interpolated_simulated_obs, interpolated_real_obs = interpolate_obs(
    simulated_obs_single_pass, real_obs_single_pass
)
...
linear_fit = LinearRegression().fit(
    interpolated_times.reshape((-1, 1)), first_residual_obs
)

# Retrieve fit model
fit = linear_fit.predict(
    np.linspace(
        interpolated_times[0], interpolated_times[len(interpolated_times) - 1]
    ).reshape((-1, 1))
)

# Compute second residual after removing linear fit
second_residual_obs = first_residual_obs - linear_fit.predict(
    interpolated_times.reshape((-1, 1))
)
