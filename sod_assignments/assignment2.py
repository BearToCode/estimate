...
data = [
    "Delfi-C3_32789_202004011044.csv",
...
recording_start_times = extract_recording_start_times_yml(
    metadata_folder, [metadata[i] for i in indices_files_to_load], old_yml=False
)
...
bodies = define_environment(
    mass, ref_area, drag_coef, srp_coef, "Delfi", multi_arc_ephemeris=False
)
...
bodies = define_environment(
    mass, ref_area, drag_coef, srp_coef, "Delfi", multi_arc_ephemeris=True
)
...
Doppler_models = dict(
    constant_absolute_bias={"activated": True, "time_interval": bias_definition},
    linear_absolute_bias={"activated": True, "time_interval": bias_definition},
)
observation_settings = define_observation_settings(
    "Delfi", Doppler_models, passes_start_times, arc_start_times
)
...
estimator = estimation_analysis.Estimator(
    bodies, parameters_to_estimate, observation_settings, multi_arc_propagator_settings
)

# Simulate (ideal) observations
ideal_observations = simulate_observations_from_estimator(
    "Delfi", observation_times, estimator, bodies
)
...
pod_output = run_estimation(
    estimator, parameters_to_estimate, observations_set, nb_arcs, nb_iterations
)
...
gravitational_parameter = bodies.get(
    "Earth"
...
for arc in range(nb_arcs):
    ...
    pos_error = np.sqrt(
        (updated_parameters[arc * 6 + 0] - arc_wise_initial_states[arc][0]) ** 2
        + (updated_parameters[arc * 6 + 1] - arc_wise_initial_states[arc][1]) ** 2
        + (updated_parameters[arc * 6 + 2] - arc_wise_initial_states[arc][2]) ** 2
    )
    print(
    ...
    state_keplerian = element_conversion.cartesian_to_keplerian(
        updated_parameters[arc * 6 : (arc + 1) * 6], gravitational_parameter
    )
    ...
    TLE_keplerian = element_conversion.cartesian_to_keplerian(
        arc_wise_initial_states[arc], gravitational_parameter
    )
    ...
    print(
        "Right Ascension of Ascending Node = \t", np.rad2deg(TLE_keplerian[4]), "\t deg"
    )
...
print(
    updated_parameters[
        6 * nb_arcs + number_of_passes + 1 : 6 * nb_arcs + number_of_passes * 2
    ]
)
...
ax.plot(
    TLE_orbit[:, 0] - TLE_orbit[0, 0],
    (TLE_orbit[:, 1] - estimated_orbit[:, 1]) / 1000,
    color="blue",
    linestyle="-.",
)
...
ax.plot(
    TLE_orbit[:, 0] - TLE_orbit[0, 0],
    (TLE_orbit[:, 2] - estimated_orbit[:, 2]) / 1000,
    color="blue",
    linestyle="-.",
)
ax.set_xlabel("Time [s]")
...
ax.plot(
    TLE_orbit[:, 0] - TLE_orbit[0, 0],
    (TLE_orbit[:, 3] - estimated_orbit[:, 3]) / 1000,
    color="blue",
    linestyle="-.",
)
ax.set_xlabel("Time [s]")
...
ax.plot(
    TLE_orbit[:, 0] - TLE_orbit[0, 0],
    (TLE_orbit[:, 4] - estimated_orbit[:, 4]) / 1000,
    color="blue",
    linestyle="-.",
)
ax.set_xlabel("Time [s]")
...
ax.plot(
    TLE_orbit[:, 0] - TLE_orbit[0, 0],
    (TLE_orbit[:, 5] - estimated_orbit[:, 5]) / 1000,
    color="blue",
    linestyle="-.",
)
ax.set_xlabel("Time [s]")
...
ax.plot(
    TLE_orbit[:, 0] - TLE_orbit[0, 0],
    (TLE_orbit[:, 6] - estimated_orbit[:, 6]) / 1000,
    color="blue",
    linestyle="-.",
)
ax.set_xlabel("Time [s]")
...
ax.plot(
    TLE_orbit[:, 1], TLE_orbit[:, 2], TLE_orbit[:, 3], label="TLE orbit", linestyle="-."
)
ax.plot(
    estimated_orbit[:, 1],
    estimated_orbit[:, 2],
...
range_estimated = np.sqrt(
    estimated_orbit[:, 1] ** 2 + estimated_orbit[:, 2] ** 2 + estimated_orbit[:, 3] ** 2
)

Vmag_TLE = np.sqrt(TLE_orbit[:, 4] ** 2 + TLE_orbit[:, 5] ** 2 + TLE_orbit[:, 6] ** 2)
Vmag_estimated = np.sqrt(
    estimated_orbit[:, 4] ** 2 + estimated_orbit[:, 5] ** 2 + estimated_orbit[:, 6] ** 2
)
...
ax.plot(
    TLE_orbit[:, 0] - TLE_orbit[0, 0],
    (range_TLE - range_estimated) / 1000,
    color="blue",
    linestyle="-.",
)
ax.set_xlabel("Time [s]")
...
for i in range(len(TLE_orbit[:, 0])):
    ...
    current_estimated_keplerian = element_conversion.cartesian_to_keplerian(
        current_estimated_state, bodies.get("Earth").gravitational_parameter
    )
...
ax.plot(
    TLE_orbit[:, 0] - TLE_orbit[0, 0],
    keplerian_difference_wrt_tle[:, 1] / 1000,
    color="blue",
    linestyle="-.",
)
ax.set_xlabel("Time [s]")
...
ax.plot(
    TLE_orbit[:, 0] - TLE_orbit[0, 0],
    np.degrees(keplerian_difference_wrt_tle[:, 3]),
    color="blue",
    linestyle="-.",
)
ax.set_xlabel("Time [s]")
...
ax.plot(
    TLE_orbit[:, 0] - TLE_orbit[0, 0],
    np.degrees(keplerian_difference_wrt_tle[:, 4]),
    color="blue",
    linestyle="-.",
)
ax.set_xlabel("Time [s]")
...
ax.plot(
    TLE_orbit[:, 0] - TLE_orbit[0, 0],
    np.degrees(keplerian_difference_wrt_tle[:, 5]),
    color="blue",
    linestyle="-.",
)
ax.set_xlabel("Time [s]")
...
ax.plot(
    TLE_orbit[:, 0] - TLE_orbit[0, 0],
    np.degrees(keplerian_difference_wrt_tle[:, 6]),
    color="blue",
    linestyle="-.",
)
ax.set_xlabel("Time [s]")
