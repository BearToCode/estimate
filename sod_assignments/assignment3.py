...
delfi_tle = environment.Tle(
    "1 32789U 08021G   20090.88491347 +.00001016 +00000-0 +70797-4 0  9997",
...
arc_start_times, arc_mid_times, arc_end_times = get_arc_times_definition(
    initial_epoch, final_epoch, arc_duration
)
...
bodies = define_environment(
    mass, ref_area, drag_coef, srp_coef, "spacecraft", multi_arc_ephemeris=True
)
...
global_orbit = propagate_initial_state(
    initial_state,
    initial_epoch,
    final_epoch,
    bodies,
...
arc_wise_initial_states = retrieve_arc_wise_states_from_orbit(
    global_orbit, arc_mid_times
)
...
stations_names = create_ground_stations(
    bodies, nb_fake_stations, stations_long, stations_lat
)
...
for obs in sorted_obs_collection.values():
    for i in range(len(obs)):
        plt.scatter(
            (np.array(obs[i][0].observation_times) - initial_epoch) / 3600.0,
            obs[i][0].concatenated_observations,
            label=stations_names[i],
        )
plt.grid()
...
if use_next_tle_as_perturbation:
    ...
    next_delfi_ephemeris = environment.TleEphemeris(
        "Earth", "J2000", next_delfi_tle, False
    )
    perturbed_arc_wise_initial_states = []
    for time in arc_mid_times:
        perturbed_arc_wise_initial_states.append(
            next_delfi_ephemeris.cartesian_state(time)
        )
...
if use_manual_perturbation:
    ...
    manual_state_perturbation = np.concatenate(
        (
            manual_position_perturbation * np.ones(3),
...
for i in range(parameters_history.shape[1]):
    ...
    true_to_formal_errors_history[:, i] = np.divide(
        true_errors_history[:, i], formal_errors
    )
...
plt.scatter(
    np.arange(0, nb_parameters), true_to_formal_errors_history[:, -1], color="blue"
)
plt.plot(
    np.arange(0, nb_parameters),
...
ax1.scatter(
    (np.array(observation_times) - initial_epoch) / 3600.0,
    residual_history[:, 0],
    color="blue",
    label="residuals",
)
ax1.plot(
    (np.array(observation_times) - initial_epoch) / 3600.0,
    noise_level * np.ones(len(observation_times)),
    color="red",
...
ax1.plot(
    (np.array(observation_times) - initial_epoch) / 3600.0,
    3 * noise_level * np.ones(len(observation_times)),
    color="red",
    linestyle="dotted",
...
plt.plot(
    -1.0 * noise_level * np.ones(2),
    [0, len(observation_times) / 10.0],
    color="red",
    linestyle="solid",
    label="1sigma noise level",
)
